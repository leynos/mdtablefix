//! Observer-aware inline wrapping orchestration.
//!
//! This submodule owns the span-grouping and line-fitting pipeline that turns a
//! segmented inline token stream into wrapped Markdown lines while keeping code
//! spans, links, and footnote references atomic. It is separated from
//! `inline.rs` so that module can stay focused on the public wrapping surface
//! and remain within the repository's 400-line file limit.
...
/// helpers. It centralises the observer wiring by translating inline
