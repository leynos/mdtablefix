...
#[test]
fn test_wrap_link_reference_definitions_passthrough_single() {
...
#[test]
fn test_wrap_link_reference_definitions_passthrough_issue_block() {
    let input = lines_vec![
...
#[test]
fn test_wrap_link_reference_definitions_passthrough_bare_url() {
...
#[test]
fn test_wrap_link_reference_definitions_passthrough_with_title() {
...
fn test_wrap_link_reference_definitions_mixed_with_paragraph() {
    ...
    assert_eq!(
        output.last().expect("output ends with link reference"),
        &input[2]
    );
    assert!(output.len() > 3);
...
fn test_wrap_link_reference_definition_at_document_start() {
    let input = lines_vec!["[first]: <https://example.com/first>", "Short paragraph.",];
    let output = process_stream(&input);
    assert_eq!(output[0], input[0]);
}
...
fn test_wrap_link_reference_definition_at_document_end() {
    let input = lines_vec!["Short paragraph.", "[last]: <https://example.com/last>",];
    let output = process_stream(&input);
    assert_eq!(
        output.last().expect("output ends with link reference"),
        &input[1]
