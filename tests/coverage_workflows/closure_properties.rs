...
impl CallGraph {
    ...
    fn len(&self) -> usize { self.starts_on_pull_request.len() }

    /// Renders workflow `i` as YAML.
    fn workflow(&self, i: usize) -> String {
        let n = self.len();
        let trigger = if self.starts_on_pull_request[i] {
            "pull_request"
        } else {
            "workflow_call"
        };
        let jobs: String =
            (0..n)
                .filter(|j| self.calls[i * n + j])
                .fold(String::new(), |mut jobs, j| {
                    let prefix = CALL_PREFIXES[self.spellings[i * n + j]];
                    let _ = writeln!(
                        jobs,
    ...
    fn expected_closure(&self) -> BTreeSet<String> {
        let n = self.len();
        let mut reach = self.calls.clone();
        for i in 0..n {
            reach[i * n + i] = true;
        }
        for k in 0..n {
        ...
        (0..n)
            .filter(|&j| (0..n).any(|i| self.starts_on_pull_request[i] && reach[i * n + j]))
