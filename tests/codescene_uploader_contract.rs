...
fn the_checksum_refresh_workflow_is_absent() {
    ...
    assert!(
        !refresh.exists(),
        "{REFRESH_WORKFLOW} maintains {DEPRECATED_VARIABLE}, which no workflow reads; delete it \
         rather than keeping a dispatch that writes an unread repository variable"
    );
}
