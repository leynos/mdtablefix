...
use mdtablefix::{
    wrap::bench_internals::{BENCH_WIDTH, large_inline_paragraph, realistic_markdown_document},
    wrap_text,
};
...
fn realistic_document_fixture_covers_and_preserves_constructs() {
    ...
    // The specific constructs survive wrapping intact.
    assert!(
        wrapped.contains("`inline-code-0`"),
        "code span split: {wrapped}"
    );
    assert!(
        wrapped.contains("(https://example.com/crate/0)"),
        "link split: {wrapped}"
    );
    assert!(wrapped.contains("[^note0]"), "footnote split: {wrapped}");
}
...
fn large_inline_paragraph_fixture_covers_and_preserves_constructs() {
    ...
    assert!(wrapped.contains("`code-0`"), "code span split: {wrapped}");
    assert!(
        wrapped.contains("(https://example.com/path/0)"),
        "link split: {wrapped}"
    );
    assert!(wrapped.contains("[^fn0]"), "footnote split: {wrapped}");
}
