...
//! a source line can carry, the two former placeholders included; and paragraphs
//! end lines with hard breaks or wrap around inline code spans longer than the
//! wrap width.
...
/// come from the ASCII range and half from the whole `char` range: a single code
/// point in a space of over a million is otherwise effectively unreachable, and
/// with it the two characters that stood in for an escaped pipe until issue #482
/// removed them.
...
///
/// Pipes are escaped as `\|` so the row stays a single row: `split_cells`
/// unescapes `\|` back to a literal pipe and `escape_literal_pipes` re-escapes a
/// raw one, so a cell carrying either spelling has to reflow to the same cell.
/// U+001F and U+001D enter the domain with the whole `char` range above. They
/// were the table parser's and the reflow helper's placeholders until issue #482
/// removed them, because a payload carrying either character collided with the
/// placeholder it was meant to stand for, so the table pass has to carry both
/// through as payload. The in-crate generator in `src/reflow/tests.rs` draws
/// from `any::<char>()` and can reach them too, but only by a one-in-a-million
/// draw.
...
fn element_strategy() -> impl Strategy<Value = String> {
    prop_oneof![
        ...
        2 => crate::bracket_reference_seam_strategy(),
        2 => table_strategy(),
