...
Vendored verbatim from `leynos/evert`, where it is
`docs/reliable-testing-in-rust-via-dependency-injection.md`. The copy matches
that file at `origin/main` commit `218a3921` (blob `60a8c6ff`), which is
unchanged since commit `1f3c30d` ("Add repo skeleton", 2026-06-27). Beyond this
paragraph the copy is byte-identical, so a re-sync is a plain diff against the
recorded commit.
