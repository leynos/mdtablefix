...
Vendored verbatim from `leynos/repovec-appliance`, where it is
`docs/rstest-bdd-users-guide.md`. The copy matches that file at `origin/main`
commit `2619fffb` (blob `4623cf19`), which is unchanged since commit `9df5d7b`
("Format Markdown sources", 2026-07-27). Beyond this paragraph the copy is
byte-identical, so a re-sync is a plain diff against the recorded commit.
